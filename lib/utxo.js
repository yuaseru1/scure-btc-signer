...
function estimateInput(inputType, input, opts) {
    ...
    if (inputType.txType === 'taproot') {
        ...
        if (input.tapInternalKey && !P.equalBytes(input.tapInternalKey, utils_js_1.TAPROOT_UNSPENDABLE_KEY)) {
        ...
        if (inputType.last.type === 'ms') {
            ...
            // 71 sig + 1 sighash
            inputScript = script_js_1.Script.encode([new Uint8Array(SIG_SIZE)]);
        }
        else if (inputType.last.type === 'pkh') {
            inputScript = script_js_1.Script.encode([new Uint8Array(SIG_SIZE), new Uint8Array(PUB_KEY_SIZE)]);
        }
